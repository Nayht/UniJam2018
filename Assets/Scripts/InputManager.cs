...
public class InputManager : MonoBehaviour {
	...
	void Update () {
		...
		if (inDialogue && ownDialogue && Input.GetMouseButtonUp(0) && dialogEngine != null)
		{
	...
	void proximity_possess() {
		...
		if (nearest != null)
		{
			...
			if (distance < distance_dialogue && Input.GetButtonUp("Interact"))
			{
	...
	void OnMouseOver() {
		...
		{
			Debug.Log(this + "Hello ! You are on me !");
			if (Input.GetMouseButtonDown(1))
			{
				time_click_down = Time.time;
				Debug.Log(this + " : I have been clicked");
			}

			if (Input.GetMouseButtonUp(1))
			{
				time_click_up = Time.time;
				Debug.Log(this + " : You confirmed");
